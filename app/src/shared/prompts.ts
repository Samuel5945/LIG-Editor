...
当作者明确要求换强调色/配色（如「换成橙色系」「强调色改红色」）时，用一句话说明选色理由，然后在回复最末尾单独输出：
\`\`\`cards-accent
{"accent":"#ff6b35"}
\`\`\`
...
- 构图：主体与视觉重心放在画面**右侧约三分之一**处，左侧三分之二保持安静留白（渐变、虚化或同色系低对比细节）——封面文字压在左侧，主体居中会与文字打架
- 上下各留出少量余量（会裁成 2.35:1 头图与 1:1 方图两种规格，方图取右侧主体）；色彩现代明快，画面中不出现任何文字、字母、字幕或水印
