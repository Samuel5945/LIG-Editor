...
3. **封面模板库**
4. **选题泳道看板**：补上"选题状态流转"维度（现只有排期维度）
5. **抠图手工精修**：画笔/橡皮 + 多参数
6. **Skill 市场**：目录/搜索/评分
7. **发文节奏 / 封面尺寸偏好**入账号预设：本次刻意未加，因为当前没有消费方，加了就是死字段
8. 长图导出（待定）；Markdown 导出（建议明确不做）
...
8. `npm run typecheck` 与 `npm run test` 全绿（当前 16 文件 / 277 例）
