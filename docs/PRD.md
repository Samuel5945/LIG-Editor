...
超出 MVP 的额外交付：分类体系与排版调性系统、排版反向导入、贴图卡片、多平台分发、Word/PDF 交稿、官方 API 直推草稿、分类级公众号凭据（多账号 + 分类绑定，按工程分类解析推送账号）、内容日历排期（含拖拽落点高亮反馈）、选题看板（四道状态泳道，状态由工程推导）、多账号预设闭环（分类级写作 Skill + 默认分发平台，新工程自动继承）、封面模板库（四套版式 + 双比例直出）、抠图手工精修（保留/擦除画笔蒙版）、联网检索、Skill 在线导入、.md 文件关联直开、昼夜配色、新手引导 Tour、版本更新入口。
