...
- 版本：**v1.1（2026-09-12，对齐已交付的 v0.6.1）**
...
├── covers/           # 封面模板渲染源（cover-wide.html / cover-square.html），可改后重渲染
...
```json
{
  ...
  "cover": { "main": "assets/cover-235.png", "square": "assets/cover-11.png", "template": "band" },
...
**改分类 = 换账号，账号级默认按「只补空缺、不覆盖」回填。** 改分类后推送账号与排版调性都按新分类立刻生效（两者都是现算），风格若不跟着走就不一致；因此改分类会把工程**缺失**的账号级默认补上（当前只有写作 Skill），而用户已在工程上手选过的值一律不动。新建工程的预设注入走同一条规则（`presetFillFor`），避免两处规则各自演化。
...
- **收尾给出有序的下一步**：成文后面板列出「① 配图 → ② 标题与封面 → ③ 审阅」，
  每步一句说明 + 直达按钮；配图那步会报出正文里还有几处 fig-suggest 占位，
  点回正文即可在占位卡上逐处出图并原位替换。两步都复用既有页签，不另造界面
...
- 封面两条产出路径，出口相同（`assets/cover-235.png` 与 `cover-11.png`）：
  1. **模板直出**（`shared/coverTemplates.ts`，纯 CSS/SVG、不依赖内置图片资源）：四套版式
     **左文右图（默认）/ 大字版式 / 左侧竖栏 / 杂志留白**，填标题与副标题、可选底图，
     离屏渲染出 **2.35:1 主封面（1175×500）+ 1:1 朋友圈小图（800×800）**；渲染源留在工程 `covers/` 下可改后重渲染。
     版式依据跨平台封面研究（少/大/粗 + 高对比 + 安全区）与公众号首图规格，七条硬规则：
     - **文字锁左区**（宽 = 画布宽 − 画布高），右侧留一个方形画面区。公众号后台的 1:1 裁剪框可拖动，
       把它落在右区即可——头条大图看文字、信息流缩略图看画面，不必互相牺牲
     - **底图同化**：有底图时照片整幅铺满，不再分「左面板 / 右图块」（两块各有底色就必然留一条边界，
       柔化层只能把硬边改成软边）。只叠一层极淡强调色（`mix-blend-mode: color`，取色相不改明度，
       右区原图亮度不受影响仍可直裁 1:1）拉齐色调。**不再压暗部渐变**——底图左侧的干净留白由提示词保证，
       再压一层暗部等于凭空造出一块与照片对不上色的面板
     - **字色自适应底图**：渲染前用 sharp 按 CSS `cover` + 定位反推出文字实际压在源图上的那块区域，
       取其平均亮度（浅于 0.62 用深字，否则用白字），可读性靠字色与一层极轻投影解决，不靠压底色。
       文字区取样而非整图取样，是因为整图平均会被右侧主体带偏
     - **底图构图由生图提示词保证**：封面提示词要求主体与视觉重心落在右侧约 1/3、左侧 2/3 保持安静留白
       （主体居中会与左侧文字打架）；1:1 方图从横图裁两侧，故取 78% 靠右取景保住主体
     - **顶部约 20% 让给标题遮罩**、底部为品牌行留净空，文字块只在这段净空内垂直居中
     - **字号受列宽硬约束**（`字号 ≤ 可用列宽 / 最长一行字数`，永不溢出画布）；支持用 `|` 手动分行，
       行越短字号越大——这是小屏可读的前提。没写 `|` 时按标点贪心折行
     - **纯色版式白字压深底**：无底图时中亮度强调色先压深一档再配白字（避免"隐身色"），最多一层极轻投影，不做 3D 描边；有底图时字色由上面的取样决定
     「杂志留白」是深色字压浅底的纯文字版式，**刻意不吃底图**——否则 1:1 上文字直接落在照片上会崩对比度
  2. **图片裁剪**：AI 生图 / 代码绘图 / 导入图 → 内置裁切器拖选裁切位置出同两个比例
  两条路径都写 `project.json.cover`，模板路径额外记下 `cover.template` 供下次沿用与追溯
- 封面提示词可直接从当前正文提炼
...
中栏「选题看板」页签，四道泳道按流转顺序排列：**待立项 → 已立项 → 已排期 → 已成稿**。

**状态不落库文件、不手工维护**——给 `idea-inbox.md` 加一个状态字段，等于多维护一份会与工程现实脱节的标记；而选题是否被用起来，本来就是工程现实的结果，现算即可：

| 状态 | 判据 |
|---|---|
| 待立项 | 找不到对应工程 |
| 已立项 | 有对应工程，未排期 |
| 已排期 | 有对应工程且有 `plannedAt`，工程仍在写 |
| 已成稿 | 对应工程已进 `reviewing` / `ready` |

对应工程的匹配顺序（`shared/ideaStage.ts`，纯函数可单测）：
1. `topic.angle + topic.audience` 全等——日历上「拖选题到日期」立项时由机器写入 `project.json.topic`，是权威关联
2. 工程名**精确等于按立项规则复算出的候选名**：`sanitizeProjectName(标题)`，或同名去重时的 `sanitizeProjectName(「标题 MMDD」)`（MMDD 取自该工程排期日期）。刻意不用「以标题开头」的前缀猜——那会把标题为「iPhone」的选题认到叫「iPhone 15 评测」的工程上，看板就会显示假状态

已排期那道按发布日先后排序，其余按评分降序；卡片可「打开工程」直连命中工程，未立项的给「去日历排期」一步跳过去。左栏选题库仍是列表视图，两者共用同一份库数据。
...
抠图现状：三种算法（**白底反预乘 / 形状掩码 / 拟合圆**）各配一个阈值滑杆（噪声底 / 掩码阈值 / 半径缩放），120ms 防抖实时预览 + 棋盘透明底；算法之上还有一层**手工精修蒙版**——保留画笔（补回算法误删的前景，颜色取原图）与擦除画笔（去掉误留背景），笔刷半径可调、边缘带软过渡、拖动按线段插值不断线。蒙版只覆盖 alpha、不改算法参数，因此调滑杆与涂抹互不干扰，**换算法后涂抹依然保留**。批量多图导入时不做抠图。
...
超出 MVP 的额外交付：分类体系与排版调性系统、排版反向导入、贴图卡片、多平台分发、Word/PDF 交稿、官方 API 直推草稿、分类级公众号凭据（多账号 + 分类绑定，按工程分类解析推送账号）、内容日历排期（含拖拽落点高亮反馈）、选题看板（四道状态泳道，状态由工程推导）、多账号预设闭环（分类级写作 Skill + 默认分发平台，新工程自动继承）、封面模板库（四套版式 + 双比例直出）、抠图手工精修（保留/擦除画笔蒙版）、联网检索、Skill 在线导入、.md 文件关联直开、昼夜配色、新手引导 Tour、版本更新提醒（双源检测：官网 update.json 主源 + GitHub Release 兜底，启动静默检查 + 顶栏手动检查，弹窗四下载入口 + 忽略版本，发版流程见 docs/release.md）。
...
| 选题库看板视图 | ✅ 已交付 | 两个不同维度都已具备：日历排期看板（§7.8）与选题状态泳道看板（§7.9） |
| 抠图交互精修 | ✅ 已交付 | 三算法 + 滑杆 + 防抖预览 + 手工精修蒙版（保留/擦除画笔，跨算法保留，§8） |
| Skill 市场/在线导入 | 🟡 部分 | 在线导入四源 + 镜像竞速已交付；无市场（目录/浏览/搜索/评分） |
| 封面模板库 | ✅ 已交付 | 四套纯 CSS 版式，两种比例各渲染一次；强调色随文章排版调性（§7.5） |
| 向导式新手流程 | 🟡 部分 | 脑暴面板已把 选题→大纲→成文 串成一路，成文后给出有序的「① 配图 ② 标题与封面 ③ 审阅」清单（含正文配图占位计数）并直达对应页签；但仍是既有面板 + 页签跳转，不是带进度指示的独立向导 |
| Markdown 导出 | ❌ 未做 | 判定为**低优先**：`article.md` 本身即事实源，需求已被"文件即产物"消解 |
| 长图导出 | ❌ 未做 | 无实现；贴图卡片是逐张固定尺寸 PNG，不是整文竖图 |

### 11.3 三期候选（按当前判断排序）

1. **头条/百家画像真正分化**：现共用 `liteHtml`，已预留独立 id
2. **独立创作向导**（原二期"向导式新手流程"的剩余部分）：现在流程已串通（脑暴面板 + 收尾三步清单 + 页签跳转），缺的是带步骤进度、把配图与封面也纳入同一界面的单一路径形态
3. **Skill 市场**：目录/搜索/评分
4. **发文节奏 / 封面尺寸偏好**入账号预设：本次刻意未加，因为当前没有消费方，加了就是死字段
5. 长图导出（待定）；Markdown 导出（建议明确不做）
...
8. `npm run typecheck` 与 `npm run test` 全绿（当前 19 文件 / 325 例）
