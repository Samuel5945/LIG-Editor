...
- 封面两条产出路径，出口相同（`assets/cover-235.png` 与 `cover-11.png`）：
  1. **模板直出**：从内置版式模板（大字版式 / 底部色带 / 左侧竖栏 / 杂志留白）选一套，填标题与副标题，
     可选把已有图片存为底图，离屏渲染出 **2.35:1 主封面（1175×500）+ 1:1 朋友圈小图（800×800）**。
     强调色取自文章排版调性，模板为纯 CSS/SVG、不依赖内置图片资源；渲染源留在工程 `covers/` 下可改后重渲染
